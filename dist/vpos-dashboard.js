...
const humanSize_1 = require("./snippets/humanSize");
const getContentType_1 = require("./snippets/getContentType");
...
const getDisks = async () => {
    ...
    for (const line of rows) {
        ...
        if (parts.length < 6)
            continue;
        const [filesystem, blocksStr, usedStr, availStr, usePercent, mountpoint] = parts;
        ...
        disks.push({
            ...
            usedKB,
            availKB,
            size: (0, humanSize_1.HumanizeKB)(sizeKB),
            used: (0, humanSize_1.HumanizeKB)(usedKB),
            avail: (0, humanSize_1.HumanizeKB)(availKB),
            usePercent,
            mountpoint,
        });
    }
...
const getLoad = async () => {
    ...
    const match = stdout.match(/load average:\s*([\d.]+),\s*([\d.]+),\s*([\d.]+)/);
    if (!match)
        throw new Error("Unexpected 'uptime' output");
...
const getTopProcesses = async (limit = 5) => {
    try {
        ...
        for (const line of rows) {
            ...
            if (!Number.isFinite(pid) || !Number.isFinite(vsz))
                continue;
            procs.push({
                pid,
                user,
...
const getCpu = async () => {
    try {
        ...
        if (nums.length < 4)
            throw new Error("Not enough CPU fields");
...
const getDirectoryUsage = async (targetPath) => {
    ...
    return {
        path: targetPath,
        usedKB,
        usedHuman: (0, humanSize_1.HumanizeKB)(usedKB),
    };
};
// Subdirectories of /opt/fccapps/vpos-perm
const getTrackedDirectories = async () => {
...
const getDirectoryChildrenUsage = async (baseDir) => {
...
const getStats = async () => {
    const [hostname, { memory, swap }, disks, load, processes, directories, cpu, temperatures,] = await Promise.all([
        getHostname(),
        getMemory(),
        getDisks(),
        getLoad(),
...
const server = http.createServer(async (req, res) => {
    try {
        ...
        fs.stat(fullPath, (err, stats) => {
            ...
            res.statusCode = 200;
            res.setHeader("Content-Type", (0, getContentType_1.getContentType)(fullPath));
            stream.pipe(res);
        });
    }
    catch (err) {
